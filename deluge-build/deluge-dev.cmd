...
@curl.exe -C - -O https://www.python.org/ftp/python/%pythonVersion%/python-%pythonVersion%-amd64.exe || @echo ERROR cURL returned an error downloading the Python installer!
...
@pip.exe install wheel || @echo ERROR installing wheel using pip!

@echo Install additional packages necessary using pip
@pip.exe install pycairo-1.19.1-cp38-cp38-win_amd64.whl		|| @echo ERROR installing local pycairo using pip!
@pip.exe install PyGObject-3.36.0-cp38-cp38-win_amd64.whl	|| @echo ERROR installing local PyGObject using pip!
@pip.exe install pygeoip									|| @echo ERROR installing PyGeoIP using pip!
@pip.exe install future										|| @echo ERROR installing future using pip!
@pip.exe install requests									|| @echo ERROR installing windows-curses using pip!

@echo Install windows-curses to make deluge-console usable
@pip.exe install windows-curses || @echo ERROR installing windows-curses using pip!
...
@pip.exe install git+https://github.com/deluge-torrent/deluge
...
curl https://github.com/deluge-torrent/deluge/commit/543a91bd9b06ceb3eee35ff4e7e8f0225ee55dc5.patch | patch.exe -d "%programfiles%/deluge/Lib/site-packages" -p1 --no-backup-if-mismatch
